...
import { createContact, updateContact, Contact } from '@/lib/actions/contact-actions'
...
export function ContactModal({ isOpen, onClose, accounts, contact }: ContactModalProps) {
    ...
    async function handleSubmit(formData: FormData) {
        ...
        startTransition(async () => {
            try {
                ...
                setError(`Failed to ${contact ? 'update' : 'create'} contact. Please try again.`)
            }
        })
    }
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md overflow-hidden flex flex-col max-h-[90vh]">
                <div className="flex justify-between items-center p-6 border-b border-slate-100 flex-shrink-0">
                    <h2 className="text-xl font-semibold text-slate-900">{contact ? 'Edit Contact' : 'Add New Contact'}</h2>
                ...
                <form action={handleSubmit} className="p-6 space-y-4 overflow-y-auto flex-1">
                    ...
                    <div>
                        ...
                        <select
                            name="status"
                            id="status"
                            defaultValue={contact?.status || 'not_contacted'}
                            className="input-field"
                        >
